...
export function Despesas() {
  ...
  const [rows, setRows] = useState<DespesaPedreiraRow[]>([]);
  ...
  useEffect(() => {
    ...
    const run = async () => {
      try {
        const dataset = await fetchDespesasPedreira2025();
        if (!active) return;
        setRows(dataset.rows);
      } catch (requestError) {
        if (!active) return;
        setError(requestError instanceof Error ? requestError.message : "Falha ao carregar despesas.");
      } finally {
  ...
  const totalDespesa = useMemo(
    () => filteredRows.reduce((sum, row) => sum + parseMoneyValue(row.vl_despesa), 0),
    [filteredRows],
  );
  ...
  const chartByMonth = useMemo(() => {
    ...
    filteredRows.forEach((row) => {
      ...
      map.set(key, (map.get(key) ?? 0) + parseMoneyValue(row.vl_despesa));
    });
  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-16">
      <PageHero
        title="Despesas"
        description="Painel analítico com chart e tabela baseado no dataset completo de despesas de Pedreira (2025)."
      ...
      <PageContainer className="pt-8">
        <SectionBlock title="Painel de Despesas" description="Fonte: data/despesas-pedreira-2025.csv">
          ...
          <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
            ...
            <StatKpi label="Despesa total" value={formatCurrency(totalDespesa)} />
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {paginatedRows.map((row, index) => (
                      <tr key={`${toText(row.id_despesa_detalhe, "id")}-${index}`} className="hover:bg-slate-50">
                        ...
                        <td className="px-3 py-2 text-right font-semibold">{formatCurrency(parseMoneyValue(row.vl_despesa))}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
